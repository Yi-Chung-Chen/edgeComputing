...
ue_speed = 28 # m/s -> 100 km/hr
total_energy_consumption = 0

# --- bs1 ----500---- bs2 ----500---- bs3
server_position = [0, 500, 1000]

i = 0
serverNo = 0
ec = EdgeComputing(R=200, W=5, np=3, C=1000, wl=0.115)
do_migration = False
while ue_speed*i<server_position[-1]:
    ...
    total_energy_consumption += ec.communication_energy_consumption(abs(ue_speed*i-server_position[serverNo]))
    
    print(f'當前服務基站bs{serverNo+1}')
    print(f'ue位置:{ue_speed*i}，與bs{serverNo+1}的距離:{abs(ue_speed*i-server_position[serverNo])}')
    print(f'當前總能耗:{total_energy_consumption}')
